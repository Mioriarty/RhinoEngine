...
namespace rhino {
	...
	class Input {
		...
		private:
			static KeyState status[];
			//  0 -   7: mouse buttons
			// 32 - 348: key states

			static float mouseX, mouseY, scrollX, scrollY;

			static std::stack<int> changedStates;

			static void _keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods);
			static void _curserPosCallback(GLFWwindow* window, double x, double y);
		...
		public:
			...
			static float* getCurserPos();
			static float* getScroll();

			static void getCurserPos(float& x, float& y);
			...
			enum class KeyState :unsigned char {
				RELEASED, JUST_PRESSED, PRESSED, LONG_PRESSED, JUST_RELEASED
			};
